      ...
      for (let col = 1; col < board.size; col++) {
        ...
        if (tile != null) {
          ...
          if (col > 0 && tile.number == this.grid[row][col - 1].number) {
            merge(tile, this.grid[row][col - 1]);
         }
        }
      }
    }
    ...
    for (let row = 0; row < board.size; row++) {
      for (let col = board.size - 2; col >= 0; col--) {
        ...
        if (tile != null) {
          ...
          if (col < board.size - 1 && tile.number == this.grid[row][col + 1].number) {
            merge(tile, this.grid[row][col + 1]);
         }
        }
      }
    ...
    for (let col = 0; col < board.size; col++) {
      for (let row = 1; row < board.size; row++) {
        ...
        if (tile != null) {
          ...
          if (row > 0 && tile.number == this.grid[row - 1][col].number) {
    ...
    case 'down':
      for (let col = 0; col < board.size; col++) {
        ...
        if (tile != null) {
          ...
          if (row < board.size - 1 && tile.number == this.grid[row + 1][col].number) {
            merge(tile, this.grid[row + 1][col]);
         }
        }
      }
